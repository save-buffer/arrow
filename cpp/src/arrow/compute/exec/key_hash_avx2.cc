...
uint32_t Hashing32::hash_varlen_avx2(bool combine_hashes, uint32_t num_rows,
                                     const uint32_t* offsets,
                                     const uint8_t* concatenated_keys, uint32_t* hashes,
                                     uint32_t* hashes_temp_for_combine) {
  if (combine_hashes) {
    return hash_varlen_imp_avx2<uint32_t, true>(num_rows, offsets, concatenated_keys,
                                                hashes, hashes_temp_for_combine);
  } else {
    return hash_varlen_imp_avx2<uint32_t, false>(num_rows, offsets, concatenated_keys,
                                                 hashes, hashes_temp_for_combine);
  }
}
...
uint32_t Hashing32::hash_varlen_avx2(bool combine_hashes, uint32_t num_rows,
                                     ...
                                     uint32_t* hashes_temp_for_combine) {
  if (combine_hashes) {
    return hash_varlen_imp_avx2<uint64_t, true>(num_rows, offsets, concatenated_keys,
                                                hashes, hashes_temp_for_combine);
  } else {
    return hash_varlen_imp_avx2<uint64_t, false>(num_rows, offsets, concatenated_keys,
                                                 hashes, hashes_temp_for_combine);
  }
}
