...
TEST(HashJoin, TrivialResidualFilter) {
  ...
  for (size_t test_id = 0; test_id < 2; test_id++) {
    ...
    HashJoinNodeOptions GenerateHashJoinNodeOptions(
        Random64Bit &rng,
        int num_left_cols,
        int num_right_cols)
    {
        ...
        for(int i = 0; i < num_left_cols; i++)
        {
            ...
            if(is_out && !is_right_join)
                opts.left_output.push_back(FieldRef(i));
        }
        for(int i = 0; i < num_right_cols; i++)
        {
            bool is_out = rng.from_range(0, 2) == 2;
            if(is_out && !is_left_join)
                opts.right_output.push_back(FieldRef(i));
        }
        ...
        for(int i = 0; i < num_keys; i++)
        {
            ...
            opts.key_cmp.push_back(
                is_or_eq ? JoinKeyCmp::IS : JoinKeyCmp::EQ);
        }
        return opts;
    }
    ...
    void TestSingleChainOfHashJoins(Random64Bit &rng)
    {
        ...
        HashJoinNodeOptions first_opt = GenerateHashJoinNodeOptions(rng, num_left_cols, num_right_cols);
        ...
        for(int i = 0; i < num_right_cols; i++)
            first_right_fields.push_back(field(std::string("r_0_") + std::to_string(i), int32(), metadata));
        ...
        for(int i = 1; i < num_joins; i++)
        {
            ...
            HashJoinNodeOptions opt = GenerateHashJoinNodeOptions(
                rng,
                opts[i - 1].left_output.size() + opts[i - 1].right_output.size(),
                num_right_cols);
            opts.push_back(std::move(opt));

            std::vector<std::shared_ptr<Field>> right_fields;
            for(int j = 0; j < num_right_cols; j++)
                right_fields.push_back(field(std::string("r_") + std::to_string(i) + "_" + std::to_string(j), int32(), metadata));
